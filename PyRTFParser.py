# -*- coding: utf-8 -*-
# Copyright (C) 2002 - 2021 Spurgeon Woods LLC
#
...
#                print "PyRTFParser.process_doc():  ORDER PROBLEM ENCOUNTERED!", self.txtCtrl.GetInsertionPoint(), self.txtCtrl.GetLastPosition()
            ...
            if "{}\\".count(c) > 0:
                ...
                if DEBUG3:
                    print ('--> ', c, self.buffer[self.index : self.index + 2], type(self.buffer[self.index : self.index + 2]), self.encoding)
                ...
                # See if we have a WORD-style Unicode character specifier, (backslash)(apostrophe) ...
                    ...
                    if DEBUG or DEBUG3:
                        print("Word-style Unicode specification:", val, self.encoding, self.fontEncoding, self.in_font_table, self.in_font_block)

                    if self.fontEncoding in ['utf8', 'cp1252']:
                        ...
                        # "smart" Apostophes are sometimes represented as /'91 and /'92, val=145 and 146
                            ...
                            self.buffer = self.buffer[:self.index] + "\x039" + self.buffer[self.index+4:]
    ...
    #                        self.index += 2
                        ...
                        # OpenOffice handles the Open Dot oddly.  This should detect that odd form
                            ...
                            self.buffer = self.buffer[:self.index] + "\xb0" + self.buffer[self.index+8:]
...
##                        print val, txt
                    ...
                    if self.in_font_block:
                        if self.fontEncoding == 'utf8':
                            # If our character value is 138 or larger ...
                            if val > 138:
...
##                                print(self.buffer)
                            ...
                            while (self.buffer[self.index] in ['\n', '\r']) or (self.buffer[self.index : self.index+2] in ["\\'", "\\\'"]):
                                # If we have a Line Break ...
                                if self.buffer[self.index] in ['\n', '\r']:
                                    # ... ignore it
                                    self.index += 1
                                # If we have a backslash and an apostrophe ...
                                    ...
                                    tmpTxt += chr(val)  # .encode(self.fontEncoding)
                            ...
                            if txt != '':
                                ...
                                if txt[0] != '*':
                                    ...
                                    if self.image_type in [wx.BITMAP_TYPE_PNG, wx.BITMAP_TYPE_JPEG]:
                                        ...
                                        stream = io.BytesIO(self.hex2int(txt).encode('latin1'))  # io.StringIO(self.hex2int(txt))
                                        # Now convert that stream to an image
                                        img = wx.Image(stream, self.image_type)  # wx.ImageFromStream(stream, self.image_type)
        ...
        try:
            ...
            # Font number specification
                ...
                if (self.fontNumber > -1) and \
                   ...
                   not (num in self.fontTable.keys()):
                    # ... see if it ends with a semicolon ...
                    if self.fontName[-1] == ';':
                        # ... and remove the semicolon
                        self.fontName = self.fontName[:-1]
                    # Add the font number / name combination to the Font Table ...
                    #  -- "name" : self.fontName.decode(self.fontEncoding) --
                    self.fontTable[self.fontNumber] = { "name" : self.fontName, "charset" : self.fontCharSet, 'encoding' : self.fontEncoding }
                ...
                if not num in self.fontTable.keys():
...
#                    self.encoding = 'utf8'
    ...
    def process_end_block(self):
        ...
        if self.in_font_table:
            ...
            if (self.fontNumber > -1) and \
               not (self.fontNumber in self.fontTable.keys()):
