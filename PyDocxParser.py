...
# Import PILLOW to handle EMF Images
from PIL import Image

# import Python's cStringIO, os, string, and sys modules
import io, os, string, sys  # cStringIO
...
# Import Transana's Dialogs module for displaying error messages
import Dialogs
...
class PyRichTextDocxHandler(richtext.RichTextFileHandler):
    ...
    def LoadFile(self, ctrl, filename, displayProgress=True):
        """ Load the contents of a Docx Format file into a wxRichTextCtrl.
            Parameters:  ctrl              a wxRichTextCtrl.  (NOT a wxRichTextBuffer.  The wxRichTextBuffer lacks methods for direct manipulation.)
                         ...
                         displayProgress   controls whether the "Importing" popup is displayed """
        if os.path.exists(filename) and isinstance(ctrl, richtext.RichTextCtrl):
            ...
            DocxTowxRichTextCtrlParser(ctrl, filename=filename, encoding=self.GetEncoding(), displayProgress=displayProgress)
...
##                self.fontAttributes[u'text'][u'fontunderlined'] = u'0'
        ...
        # If the element is an image element ...
            ...
            # (It appears to me that all images will be PNG images coming from the RichTextCtrl, but not from other sources.)
            if attributes[u'imagetype'] == u'15':  # wx.BITMAP_TYPE_PNG = 15
                ...
                print("Image of UNKNOWN TYPE!!", attributes[u'imagetype'], attributes.keys(), attributes.values())
    ...
    def endElement(self, name):
        ...
        # If we have a data element ...
            ...
            if self.elementType == 'ImagePNG':
                ...
                # UTF8 and CP1252 encodings do not work!!
                stream = io.BytesIO(self.hex2int(self.dataValue).encode('latin1'))
                # Create a run
                run = self.paragraph.add_run()
...
class DocxTowxRichTextCtrlParser:
    ...
    def process_doc(self, displayProgress=True):
        ...
        # Initialize an flag for whether an Image Import error has been shown and should not be shown again
        ...
        for p in self.document.paragraphs:
            ...
            if p.paragraph_format.line_spacing != None:
                ...
                # Sometimes, Word can provide huge line-spacing values, such as when handling converted PDFs.
                # Let's max Line Spacing out at triple spacing
                if pLineSpacing > 30:
                    pLineSpacing = 30
            else:
                # Let's default to single Line Spacing for values of None, which is 10!
                pLineSpacing = 10
                ...
                try:
                    ...
                    # If we have a picture ...
                        ...
                        for picture in r.picture_lst:
                            # ... if we have an image of a known type (PNG, JPEG, EMF) ...
                            if picture.image_type in list(imageTypes.keys()) + [u'image/x-emf']:
                                # ... get a StringIO stream to take the image data.
                                stream = io.BytesIO(picture.image_data)
                                # If we have an EMF Image ...
                                if picture.image_type == u'image/x-emf':
                                    try:
                                        # ... open a PILLOW Image, which can handle EMF data, and load the data stream
                                        pilImg = Image.open(stream)
                                        # Let's normalize the picture to the page size, assuming an 8.5 inch width and 72 DPI.
                                        # EMF images are vector images, so should resize well.  They tend, in the few examples
                                        # I've seen, to have large dimensions, so I anticipate this will be scaling down mostly.
                                        # If the picture is less than 4 inches wide or more than 6.5 inches wide ...
                                        if (picture.width < 4 * 72) or (picture.width > (6.5 * 72)):
                                            # ... determin the picture's degree of horizontal change
                                            aspectRatio = (6.5 * 72) / picture.width
                                            # Set the width to 6.5 inches
                                            width = int(6.5 * 72)
                                            # Set the height proportionally
                                            height = int(picture.height * aspectRatio)
                                            # Rescale the image using PILLOW
                                            pilImg = pilImg.resize((width, height), Image.BILINEAR)
                                        # If the picture is already an appropriate size ...
                                        else:
                                            # ... use its original size
                                            width = picture.width
                                            height = picture.height

                                        # Create an empty wx.Image that is the correct size
                                        img = wx.EmptyImage(width, height)
                                        # Copy the data from the PILLOW Image to the wx.Image, converting it along the way.
                                        img.SetData(pilImg.convert("RGB").tobytes())
                                    # Most likely, this exception will be raised if EMF is not supported
                                    except IOError as e:
                                        # If the user hasn't said to stop showing image error yet ...
                                        if not imageErrorShown:
                                            # ... create an error message ...
                                            msg = unicode(_('Unsupported image Format: "WMF/EMF" format in file\n\n%s.'), 'utf8')
                                            if 'darwin' in sys.platform:
                                                msg += u'\n\n' + _('This is a Windows image format.')
                                            # ... display the error message ...
                                            errDlg = Dialogs.ErrorDialog(None, msg % self.filename, _("Import Error"), includeSkipCheck=True)
                                            errDlg.ShowModal()
                                            # ... see if the user requests not to see more image error messages ...
                                            imageErrorShown = errDlg.GetSkipCheck()
                                            # ... and destroy the error dialog
                                            errDlg.Destroy()
                                        # Set img to None, just in case there is an image remnant
                                        img = None
                                        # We should insert a single space here so that REFI-QDA Selection positioning is still accurate
                                        self.txtCtrl.WriteText(u' ')
                                # For non-EMF images ...
                                else:
                                    # ... convert that stream to an image using wxPython rather than PILLOW
                                    img = wx.Image(stream, imageTypes[picture.image_type])
                                    if not img.IsOk():
                                        # If the user hasn't said to stop showing image error yet ...
                                        if not imageErrorShown:
                                            # ... create an error message ...
                                            msg = unicode(_('Problem importing an image of %s type in file\n\n%s'), 'utf8')
                                            # ... display the error message ...
                                            errDlg = Dialogs.ErrorDialog(None, msg % (picture.image_type, self.filename), _("Import Error"), includeSkipCheck=True)
                                            errDlg.ShowModal()
                                            # ... see if the user requests not to see more image error messages ...
                                            imageErrorShown = errDlg.GetSkipCheck()
                                            # ... and destroy the error dialog
                            ...
                            # If the image type is unknown, I can probably fix that!  Put a message in the text!!
                                ...
                                if not imageErrorShown:
                                    # ... create an error message ...
                                    msg = unicode(_('Unknown Image Type: "%s" in file\n\n%s'), 'utf8')
                                    # ... display the error message ...
                                    errDlg = Dialogs.ErrorDialog(None, msg % (picture.image_type, self.filename), _("Import Error"), includeSkipCheck=True)
                                    errDlg.ShowModal()
                                    # ... see if the user requests not to see more image error messages ...
