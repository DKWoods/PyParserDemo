...
class FormatParagraphPanel(wx.Panel):
    ...
    def __init__(self, parent, formatData):
        ...
        wx.Panel.__init__(self, parent, -1)   # , _('Font'), style=wx.CAPTION | wx.SYSTEM_MENU)
        ...
        # With wxPython 4, we can't compare paragraphLineSpacing when the value is None, so let's detect
        # and handle that here
        if self.formatData.paragraphLineSpacing is None:
        ...
        box.Add(self.errorTxt, 0, wx.EXPAND | wx.GROW | wx.LEFT | wx.RIGHT | wx.BOTTOM, 15)
    ...
    def OnAlignSelect(self, event):
        ...
        if self.lbAlign.GetStringSelection() == _("Left"):
            self.formatData.paragraphAlignment = wx.TEXT_ALIGNMENT_LEFT
        elif self.lbAlign.GetStringSelection() == _("Center"):
            self.formatData.paragraphAlignment = wx.TEXT_ALIGNMENT_CENTRE
        elif self.lbAlign.GetStringSelection() == _("Right"):
    ...
    def OnLineSpacingSelect(self, event):
        ...
        if self.lbSpacing.GetStringSelection() == _("Single"):
            self.formatData.paragraphLineSpacing = wx.TEXT_ATTR_LINE_SPACING_NORMAL
        elif self.lbSpacing.GetStringSelection() == _("11 point"):
            self.formatData.paragraphLineSpacing = 11
        elif self.lbSpacing.GetStringSelection() == _("12 point"):
            self.formatData.paragraphLineSpacing = 12
        elif self.lbSpacing.GetStringSelection() == _("One and a half"):
            self.formatData.paragraphLineSpacing = wx.TEXT_ATTR_LINE_SPACING_HALF
        elif self.lbSpacing.GetStringSelection() == _("Double"):
            self.formatData.paragraphLineSpacing = wx.TEXT_ATTR_LINE_SPACING_TWICE
        elif self.lbSpacing.GetStringSelection() == _("Two and a half"):
            self.formatData.paragraphLineSpacing = 25
        elif self.lbSpacing.GetStringSelection() == _("Triple"):
